      ...
      <div class="myresume">
        <div class="basic" id="baseInfo">
          <div class="header_img_box">
            <a-upload name="formFile" listType="picture-card" class="avatar-uploader" accept=".png, .jpg, .jpeg"
              :showUploadList="false" :action="uploadPhoto" :beforeUpload="beforeUpload" @change="handleChange"
              :headers="geHeader">
              <div v-if="headImg"><img class="headerImg" :src="headImg" alt="avatar" />
            ...
            <p style="text-align:center;margin-top:20px;">大小不超过200k 格式为jpg、png</p>
          </div>
          ...
          <div class="basic-info">
            ...
            <p>
              ...
              <span class><i class></i><span v-text="page_cityName"></span>
              </span>/
              <span class>
                <i class></i>
              ...
              <span class>
                <i class></i>
                <span> {{page_expectationSalary+'(K/年)'}}</span>
              </span>
            </p>
      ...
      <div class="mr-template work-exp" id="workExp">
        ...
        <ol class="work-exp_group">
          <li class="work-exp_list" :key="index" v-for="(item, index) in companyWorkList">
            ...
            <p class="exp-list_time">{{item.startYear}}.{{item.startMonth}}-{{item.endYear}}.{{item.endMonth}}</p>
      ...
      <div class="right-nav">
        <div class="mr_uploaded clearfixs">
          <p class="mr-upload__title">
            <strong>附件简历</strong>
            <a-upload name="formFile" listType="text" accept=".pdf, .doc, .docx" :multiple="false" :showUploadList="false"
              :action="uploadJianli" :headers="geHeader" @change="handlePDFChange">
    ...
    <a-modal title="基本信息" v-model="selfModal" :footer="null" @ok="selfModal = false">
      <a-form :form="form" @submit="handleSelfInfoSubmit">
        <a-form-item v-bind="formItemLayout">
          ...
          <a-input v-model="name" v-decorator="['name', {
              rules: [
        ...
        <a-form-item v-bind="formItemLayout">
          ...
          <a-input v-model="email" v-decorator="['email', {
            rules: [
        ...
        <a-form-item v-bind="formItemLayout">
          ...
          <a-date-picker placeholder :defaultValue="moment(birthday, 'YYYY-MM-DD')" @change="onBirthdayChange" />
        </a-form-item>
        <a-form-item v-bind="formItemLayout">
        ...
        <a-form-item v-bind="formItemLayout" class="city_set">
          ...
          <a-select placeholder="请选择省份" v-model="activeProvince" @change="getZoneList(activeProvince, true)">
            <a-select-option :key="item.code" :value="item.code" v-for="(item) in Province">{{item.name}}</a-select-option>
          </a-select>
        ...
        <a-form-item v-bind="formItemLayout" class="city_set">
          ...
          <a-select v-model="expectationOccupation">
            <a-select-option :key="index" :value="item.id" v-for="(item, index) in twoJobList">{{item.name}}</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item v-bind="formItemLayout" class="city_set">
    ...
    <a-modal title="工作经历" v-model="jianliModal" :footer="null" @ok="jianliModal = false">
      <a-form :form="form" @submit="handleWordEdit">
        ...
        <a-form-item v-bind="formItemLayout">
          ...
          <a-range-picker format="YYYY-MM-DD" :disabledDate="disabledDate" :value="workDate" @change="workDateChange" />
        </a-form-item>
    ...
    <a-modal title="项目经历" v-model="projectModal" :footer="null" @ok="projectModal = false">
      <a-form :form="form" @submit="handleProjectSubmit">
        ...
        <a-form-item v-bind="formItemLayout">
          ...
          <a-range-picker format="YYYY-MM-DD" :disabledDate="disabledDate" :value="projectDate" @change="projectDateChange" />
        </a-form-item>
      ...
      <a-form :form="form" @submit="handleStudySubmit">
        ...
        <a-form-item v-bind="formItemLayout">
          ...
          <a-select v-model="schoolEducation" @change="e=>this.schoolEducation = e">
            <a-select-option :key="index" :value="item" v-for="(item, index) in ['初中','高中','大专','本科','研究生','博士']">{{item}}</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item v-bind="formItemLayout">
          <span slot="label">学习时间</span>
          <a-range-picker format="YYYY-MM-DD" :disabledDate="disabledDate" :value="schoolDate" @change="schoolDateChange" />
        </a-form-item>
        <a-form-item style="text-align:center">>
          <a-button type="primary" html-type="submit">保存</a-button>
        </a-form-item>
...
<style lang="less" scoped>
  ...
  .persion-center-container {
    ...
    .mr-myresume-left .mr_bottom .mr_bottom_l i {
      ...
      width: 16px;
      height: 12px;
      background: url(//www.lgstatic.com/lg-www-fed/mycenter/modules/common/img/icons_mr_0ef4596.png) -90px -140px no-repeat;
    }
    ...
    .mr-myresume-left .mr_bottom .mr_down:hover i {
      background: url(//www.lgstatic.com/lg-www-fed/mycenter/modules/common/img/icons_mr_0ef4596.png) -117px -140px no-repeat;
    }
  ...
  .right-nav .mr_del {
    ...
    width: 12px;
    height: 13px;
    background: url(//www.lgstatic.com/lg-www-fed/mycenter/modules/myresume_new/img/del@2x_9e9472b.png) no-repeat;
  ...
  .right-nav .mr_more {
    ...
    width: 30px;
    height: 26px;
    background: url(//www.lgstatic.com/lg-www-fed/mycenter/modules/myresume_new/img/more_normal@2x_d67aea1.png) no-repeat;
    background-size: contain;
    cursor: pointer;
  }

  .right-nav .mr_more:hover {
    background: url(//www.lgstatic.com/lg-www-fed/mycenter/modules/myresume_new/img/more_hover@2x_98c9d2d.png) no-repeat;
    background-size: contain;
  }
  ...
  .right-nav .right-nav__content .right-nav__item i {
    ...
    width: 18px;
    height: 18px;
    background: url(//www.lgstatic.com/lg-www-fed/mycenter/modules/common/img/icons_mr_0ef4596.png) no-repeat;
  ...
  .right-nav .right-nav__content .right-nav__item .mr_social_account_i {
    background: url(//www.lgstatic.com/lg-www-fed/mycenter/modules/common/img/social_account_a5d472f.png) no-repeat;
    background-position: -5px -5px;
  }

  .right-nav .right-nav__content .right-nav__item .mr_portfolio_account_i {
    background: url(//www.lgstatic.com/lg-www-fed/mycenter/modules/myresume_new/img/photo@2x_defa2c7.png) no-repeat;
    background-size: contain;
  }
